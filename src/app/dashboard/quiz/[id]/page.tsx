...
export default function QuizPage() {
    ...
    const { taskInfo, setQuizQuestions: setContextQuizQuestions, quizQuestions, addQuizAnswer, getAnswerForQuestion, useCoin, coinsUsed, prefetchedQuizQuestions, addPenalty } = useStudySession();
    ...
    const fetchQuestions = useCallback(async () => {
        ...
        setError(null);
        try {
            // Use prefetched questions if available, otherwise generate new ones
            let questions;
            if (prefetchedQuizQuestions && prefetchedQuizQuestions.length > 0) {
                console.log('Using prefetched quiz questions');
                questions = prefetchedQuizQuestions;
            } else {
                console.log('Generating new quiz questions');
                const result = await generateQuizQuestions({ pdfDataUri: taskInfo.dataUri! });
                questions = result.questions;
            }
            setContextQuizQuestions(questions);
        } catch (error) {
    ...
    }, [taskInfo, router, toast, setContextQuizQuestions, prefetchedQuizQuestions]);
    ...
    }, [currentQuestionIndex, quizQuestions, router, params.id]);
    ...
    }, [useCoin, addPenalty, currentQuestionIndex]);
